#Esse código é igual ao checkers.py mas aqui ainda tem todos os prints de debug utilizados na criação dele.
#O que pode ajudar no entendimento do código, ou até mesmo para um futuro debug
...
def submit_move():
    ...
    #while len(list_of_moves) > 0:
        #color = board_pos[int(list_of_moves[0])][int(list_of_moves[1])]
        #board_pos[int(list_of_moves.pop(0))][int(list_of_moves.pop(0))] = 0
        #board_pos[int(list_of_moves.pop(0))][int(list_of_moves.pop(0))] = color
    test = entryPoint()
    if test:
...
if test == 1:
    ...
    print("Tudo errado, mas ainda assim certo")
