...
def start_game():
    ...
    qtd_moves = 0
    list_of_moves.clear()
    status_label.config(text="Faça sua jogada", fg="black")
    which_movement = 0
    front_end_turn = 0
...
def submit_move():
    ...
    if test[4] == 0:
        ...
        status_label.config(text="Brancas ganharam", fg="blue")
        path_to_audio = os.path.join(path_project, "audio/end.mp3")
        ih.play_audio(path_to_audio)
    elif test[4] == 4:
        b.set_end_board(test[4], board_pos)
        status_label.config(text="Negras ganharam", fg="blue")
        path_to_audio = os.path.join(path_project, "audio/end.mp3")
        ih.play_audio(path_to_audio)
    else:
        status_label.config(text=f"Jogada da máquina: {b.format_moves_to_string(test)}", fg="green")
